...
class videoController {
    ...
    public function receiveM3u8Url(){
        ...
        $qiNiu=new qiNiu();
        $receiveData=json_decode($GLOBALS['HTTP_RAW_POST_DATA'],true);
        $key=$receiveData["items"][0]["key"];
        $persistentId=$receiveData["id"];
        $code=$receiveData["code"];

        $m3u8Url="http://".$qiNiu->bucket.".".$qiNiu->qiNiuDn."/".$key;
